...
namespace DialogueQuests
{
    ...
    public class NarrativeCondition : MonoBehaviour
    {
        ...
        public bool IsMet(Actor player)
        {
            ...
            {
                int i1 = ndata.GetCustomInt(target_id);
                int i2 = target_type == NarrativeConditionTargetType.Target ? ndata.GetCustomInt(other_target_id) : value_int;
                condition_met = CompareInt(oper, i1, i2);
            }

            if (type == NarrativeConditionType.CustomFloat)
            {
                float f1 = ndata.GetCustomFloat(target_id);
                float f2 = target_type == NarrativeConditionTargetType.Target ? ndata.GetCustomFloat(other_target_id) : value_float;
                condition_met = CompareFloat(oper, f1, f2);
            }

            if (type == NarrativeConditionType.CustomString)
            {
                string s1 = ndata.GetCustomString(target_id);
                string s2 = target_type == NarrativeConditionTargetType.Target ? ndata.GetCustomString(other_target_id) : value_string;
            ...
            if (type == NarrativeConditionType.QuestProgress)
            {
                ...
                if (quest != null)
                {
                    int avalue = quest.GetQuestProgress(target_id);
            ...
            if (type == NarrativeConditionType.ActorRelation)
            {
                ...
                if (actor != null)
                {
                    int avalue = ndata.GetActorValue(actor);
                    condition_met = CompareInt(oper, avalue, value_int);
                }
            }
            ...
            if (type == NarrativeConditionType.CountSObject)
            {
                IdData item = (IdData)value_data;
            ...
            if (type == NarrativeConditionType.HasLevel)
            {
                PlayerCharacter character = player.GetComponent<PlayerCharacter>();
                int i1 = character != null ? character.SaveData.GetLevel(target_id) : 0;
                condition_met = CompareInt(oper, i1, value_int);
            }
#endif
