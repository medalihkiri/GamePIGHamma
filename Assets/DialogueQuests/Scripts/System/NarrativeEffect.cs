...
#if FARMING_ENGINE || SURVIVAL_ENGINE_ONLINE
        OpenShop=84,
        NextDay=85,
#endif
        ...
        public void Trigger(Actor player)
        {
            ...
            if (type == NarrativeEffectType.CustomInt)
            {
                if(oper == NarrativeEffectOperator.Set)
                    ndata.SetCustomInt(target_id, value_int);

                if (oper == NarrativeEffectOperator.Add)
                {
                    int value = ndata.GetCustomInt(target_id);
                    ndata.SetCustomInt(target_id, value + value_int);
                }
            }
            ...
            {
                if (oper == NarrativeEffectOperator.Set)
                    ndata.SetCustomFloat(target_id, value_float);

                if (oper == NarrativeEffectOperator.Add)
                {
                    float value = ndata.GetCustomFloat(target_id);
                    ndata.SetCustomFloat(target_id, value + value_float);
                }
            }

            if (type == NarrativeEffectType.CustomString)
            {
                ndata.SetCustomString(target_id, value_string);
            }
            ...
            if (type == NarrativeEffectType.Instantiate)
            {
                if (value_object != null && !string.IsNullOrWhiteSpace(target_id))
                {
                    Region region = Region.Get(target_id);
            ...
            if (type == NarrativeEffectType.ActorRelation)
            {
                ...
                {
                    if (oper == NarrativeEffectOperator.Set)
                        ndata.SetActorValue(actor, value_int);

                    if (oper == NarrativeEffectOperator.Add)
                    {
                        int value = ndata.GetActorValue(actor);
                        ndata.SetActorValue(actor, value + value_int);
                    }
                }
            }
            ...
            if(type == NarrativeEffectType.RemoveItem){
                PlayerCharacter character = player.GetComponent<PlayerCharacter>();
                if(character != null)
                    character.Inventory.UseItem((ItemData) value_data, value_int);
            }

            if(type == NarrativeEffectType.GainGold){
                PlayerCharacter character = player.GetComponent<PlayerCharacter>();
                if(character != null){
                    character.SaveData.gold += value_int;
