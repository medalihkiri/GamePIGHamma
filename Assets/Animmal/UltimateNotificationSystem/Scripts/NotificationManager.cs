...
namespace Animmal.NotificationSystem
{
    ...
    public partial class NotificationManager : MonoBehaviour
    {
        ...
        //Dictionary to store notifications called 'WithId' functions 
        protected Dictionary<string, NotificationStatus> ActiveIDNotifications = new Dictionary<string, NotificationStatus>();
        public Dictionary<string, NotificationStatus> GetNotifictionItemsShownWithUniqueID { get { return ActiveIDNotifications; } }
        ...
        #region NOTIFICATIONSWITHID

        public virtual NotificationStatus ShowNotificationWithID(string _NotificationID, string _StyleID, NotificationData notificationData)
        {
            if (ActiveIDNotifications.ContainsKey(_NotificationID))
            {
                if (ActiveIDNotifications[_NotificationID] != null)
                {
                    ActiveIDNotifications[_NotificationID].NotificationItem.Hide(true);
                    ActiveIDNotifications[_NotificationID] = ShowNotification(_StyleID, notificationData);
                }
            }
            else
            {
                ActiveIDNotifications.Add(_NotificationID, Instance.ShowNotification(_StyleID, notificationData));
            }
            return ActiveIDNotifications[_NotificationID];
        }

        public virtual NotificationStatus GetNotificationWithID(string _NotificationID)
        {
            if (ActiveIDNotifications.ContainsKey(_NotificationID))
            {
                return ActiveIDNotifications[_NotificationID];
            ...
            {
                Debug.LogError("Ultimate Notification System - NotificationManager function GetNotificationWithID: No notification with was found with ID " + _NotificationID);
                return null;
            }
        }

        public virtual void HideNotificationWithID(string _NotificationID)
        {
            if (ActiveIDNotifications.ContainsKey(_NotificationID))
            {
                //if we have notification cached, and it is already shown we remove it for cache and call hide command on notification Item
                if (ActiveIDNotifications[_NotificationID].NotificationItem != null)
                {
                    ActiveIDNotifications[_NotificationID].NotificationItem.Hide();
                    ActiveIDNotifications.Remove(_NotificationID);
                }
                else
                {
                    //If notificaiton is queued, we wait for it to be shown first and hide it immedeately after
                    ActiveIDNotifications[_NotificationID].StatusChangedEvent.AddListener(DelayedHideCommand);
                    ActiveIDNotifications.Remove(_NotificationID);
                }
            }
        }

        protected virtual void DelayedHideCommand(NotificationStatus _NotificationStatus)
        {
            if (_NotificationStatus.NotificationItem != null)
            {
                _NotificationStatus.StatusChangedEvent.RemoveListener(DelayedHideCommand);
                _NotificationStatus.NotificationItem.Hide(true);
            }
        }
